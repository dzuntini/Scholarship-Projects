...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/10/09 17:58:02 by dzuntini          #+#    #+#             */
/*   Updated: 2021/10/11 19:13:59 by dzuntini         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	lexer(t_data *ms, char *line)
{
	...
	wc = word_count(line);
	ms->word_tab = malloc((wc + 1) * sizeof(char *));
	i_tab = -1;
	i_str = 0;
	while (++i_tab < wc)
	{
		...
		if (is_symbol(line[i_str]))
			ms->word_tab[i_tab] = parse_symbol(line, &i_str);
		else
			ms->word_tab[i_tab] = parse_word(line, &i_str);
	}
	ms->word_tab[i_tab] = 0;
	return (check_syntax(ms->word_tab));
}
